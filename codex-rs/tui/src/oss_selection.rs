    ...
    vec![
        ...
        SelectOption {
            ...
            description: t(
                ...
                "本地 Ollama 服务器（Responses API，默认端口 11434）",
            ),
            key: KeyCode::Char('o'),
...
impl OssSelectionWidget<'_> {
    fn new(lmstudio_status: ProviderStatus, ollama_status: ProviderStatus) -> io::Result<Self> {
        ...
        let mut contents: Vec<Line> = vec![
            Line::from(vec![
                "? ".fg(Color::Blue),
                t("Select an open-source provider", "选择开源服务提供方").bold(),
            ]),
            Line::from(""),
            Line::from(t(
        ...
        contents.push(
            Line::from(t("  ● Running  ○ Not Running", "  ● 运行中  ○ 未运行"))
...
impl WidgetRef for &OssSelectionWidget<'_> {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        Line::from(if crate::is_zh_locale() {
            "选择提供方？"
        } else {
            "Select provider?"
        })
